...
- Чтобы было понятно, к чему относятся коммиты, используйте префиксы:
  - **lesson_n:** — изменения, которые вы повторяли за Григорием по ходу урока (например, `lesson_1: реализовал методы для работы со storage`)
  - **hw_n:** — изменения, связанные с вашей самостоятельной реализацией ДЗ (например, `hw_3: реализовал Шаблонный метод`)
...
- Каждый пункт ДЗ оформляйте в отдельный коммит
