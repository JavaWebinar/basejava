...
- Java 8 Streams:
    1) реализовать метод  `int minValue(int[] values)` только через стрим
который принимает массив цифр от 1 до 9, надо выбрать уникальные и вернуть минимально возможное число, составленное из этих уникальных цифр. Не использовать преобразование в строку и обратно
{1,2,3,3,2,3} вернет 123
{9,8} вернет 89

    2)  реализовать метод `List<Integer> oddOrEven(List<Integer> integers)`
если сумма всех чисел нечетная - удалить все нечетные, если четная - удалить все четные. Решение должно быть в один стрим.
