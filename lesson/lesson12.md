...
- **Java 8 Streams**:
  ...
  - реализуйте метод `List<Integer> oddOrEven(List<Integer> nums)`
    ...
    Используйте массив из предыдущего метода. Если сумма всех чисел нечетная - удалить все нечетные, если четная - удалить все четные. Сложность алгоритма должна быть O(N). С помощью Optional - решение в один стрим
