...
- Из-за того, что количество тестовых классов растет, воспользуйтесь [аннотациями](https://www.geeksforgeeks.org/software-testing/junit-5-test-suites-with-example/) JUnit, которые помогут упростить их запуск (для этого у вас в `Dependencies` для `JUnit 6` должна быть зависимость `junit-platform-suite-api`. Ссылка на нее есть в условии ДЗ четвертого урока). Отформатируйте класс-запуска тестов в соответствии с [этой](https://github.com/user-attachments/assets/11670798-14f7-41ff-a03c-dbf5a900671f) картинкой
