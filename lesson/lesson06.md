...
- Реализуйте для getAllSorted() компаратор, используя методы интерфейса [Comparator](https://www.baeldung.com/java-8-comparator-comparing) и лямбда-выражение: если `fullName` разных людей совпадает, то сортируйте дополнительно по uuid
- Из-за того, что количество тестовых классов растет, воспользуйтесь [аннотациями](http://javastudy.ru/junit/junit-suite-tests/) JUnit, которые помогут упростить их запуск (по желанию для [JUnit5](https://junit.org/junit5/docs/current/user-guide/#junit-platform-suite-engine)). Форматируйте класс-запуска тестов в соответствии с [этой](https://drive.google.com/file/d/1XHFbxNLekyAWCPJROPJcxmJydjg0K3eH/view?usp=sharing) картинкой
