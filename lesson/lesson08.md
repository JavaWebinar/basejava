...
3) Реализуйте в `ResumeTestData` метод, который будет создавать, заполнять резюме данными и возвращать его для тестирования в `AbstractStorageTest`. Все секции резюме, используемые в тестах, должны быть заполнены
