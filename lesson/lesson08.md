...
3) Реализуйте в `ResumeTestData` метод, который будет принимать uuid и fullName, создавать резюме, заполнять его данными и возвращать для тестирования в `AbstractStorageTest`. Все секции резюме, используемые в тестах, должны быть заполнены
