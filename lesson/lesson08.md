...
- <a href="https://nuancesprog.ru/p/13714/">Основы API Time для Java</a>
...
1) Переделайте модель резюме: учтите, что на одной работе (в одном учебном заведении) можно работать/учиться в разные периоды. При этом имя организации и ссылка на сайт не должны дублироваться
2) Отображайте пустую строку, если `url` равен `null`
3) Реализуйте рекурсивный обход и вывод имени файлов в директориях и поддиректориях, начиная с корня вашего проекта
4) Реализуйте в `ResumeTestData` метод, который:
   - принимает `uuid` и `fullName`
   - создает резюме
   - заполняет данными все секции резюме
   - возвращает заполненное резюме в `AbstractStorageTest`     
   (используйте данный метод для всех резюме, создаваемых в `AbstractStorageTest`)
5) Реализуйте `AbstractFileStorage` - базовый класс для хранения резюме в файлах
